...
class HighPrecisionEQAudioProcessor : public juce::AudioProcessor
{
public:
    HighPrecisionEQAudioProcessor();
    ~HighPrecisionEQAudioProcessor() override;
...
private:
    ...
    // アナライザ同期および差分リスニング用のステレオ遅延バッファ (ゼロレイテンシーのためサイズは1)
    ...
    JUCE_DECLARE_NON_COPYABLE_WITH_LEAK_DETECTOR(HighPrecisionEQAudioProcessor)
};
